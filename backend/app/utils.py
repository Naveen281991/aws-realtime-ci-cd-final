...
def render_email_template(
    *,
    ...
    """Render an HTML email template with the supplied context."""
...
def send_email(
    *,
    ...
    logger.info(
        "Sending email: to=%s subject=%s smtp=%s:%s",
        email_to,
        subject,
        settings.SMTP_HOST,
        settings.SMTP_PORT,
    ...
    logger.info(
        "SMTP response: status_code=%s status_text=%s",
    ...
    if response.status_code is not None and response.status_code >= 400:
        raise RuntimeError(
            f"SMTP rejected email: {response.status_code} {response.status_text}"
        )
...
def verify_password_reset_token(token: str) -> str | None:
    """Verify a password-reset token and return its email."""
    try:
